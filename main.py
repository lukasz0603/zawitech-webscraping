...
async def generate_embed(username: str = Form(...)):
    ...
    snippet = f"""<script src="https://zawitech-frontend.onrender.com/widget.js?client_id={embed_key}" async></script>"""
      ...
      # Wstaw do bot_generation jeśli nie istnieje
    await database.execute(
        """
        INSERT INTO bot_generation (client_id, generated, script)
        VALUES (:cid, :gen, :script)
...
@app.get("/bot/embed")
async def get_bot_script(client_id: UUID = Query(...)):
    try:
        row = await database.fetch_one(
            """
            SELECT script
            FROM bot_generation
            WHERE client_id = :cid AND generated = TRUE
            """,
            values={"cid": client_id}
        )
        if not row:
            raise HTTPException(status_code=404, detail="Bot nie istnieje")

        return {"script": row["script"]}
    except Exception as e:
        print("❌ /bot/embed error:", e)
